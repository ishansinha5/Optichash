...
While there is a steep learning curve to enterprise Java, utilizing Spring Boot provided the exact robustness and thread management needed to juggle heavy binary file streams across multiple internal containers.
...
* **Stage 2 (The Runner):** Rather than pushing the massive Maven image to production, the `Dockerfile` switches to an ultra-lightweight `eclipse-temurin:17-jre-jammy` image, copying *only* the compiled `.jar`. This drastically reduces the final image size and cleanly exposes port `8080` to our Nginx proxy layer.